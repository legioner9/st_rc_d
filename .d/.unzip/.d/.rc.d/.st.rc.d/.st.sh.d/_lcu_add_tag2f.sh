...
_lcu_add_tag2f() {
    ...
    local hint="hint->add tag \$1 to file.lcu \$2"
    local item=
    ...
    if [[ "-h" == "$1" ]]; then
        echo -e "
MAIN: ${FNN} :: add tag \$1 to file.lcu \$2
TAGS: 
\$1 
[, \$2]
CNTL: 
...
"
    ...
    local str_tags=$(_lcu2e $ptr_path_2 _s | grep TAGS:)

    str_tags=$(_sTRMe "$str_tags")

    echo -e "${GREEN}\$str_tags = '$str_tags'${NORMAL}" #print variable

    eval "_s2f \"${str_tags}\" \"${str_tags} @${1}\" ${ptr_path_2}"
    # eval "_s2f \"${str_tags}\" \"${str_tags} @${1}\" ${ptr_path_2}"

    # echo -e "${HLIGHT}--- sed -i 's|${str_tags}|${str_tags}_|g' ${ptr_path_2} ---${NORMAL}" #start files
    # eval "sed -i 's|${str_tags}|${str_tags}_|g' ${ptr_path_2}"

    #* ${HOME}.d/.rc.d/.st.rc.d/.st.d

    #? ----- END _lcu_add_tag2f body -----

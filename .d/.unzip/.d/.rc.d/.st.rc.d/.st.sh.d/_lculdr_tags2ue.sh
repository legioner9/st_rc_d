...
_lculdr_tags2ue() {
    ...
    local hint="hint->\$1 u@path dir_with_file.lst \$2 (0|num_menu - file in dir)"
    local item=
    ...
    if [[ "-h" == "$1" ]]; then
        echo -e "
MAIN: ${FNN} :: stdout stdout all unique lcu tags like arr from menu : .lst file with lcu_dir : flow _lcudr_tags2ue any_str in .lst
TAGS: 
\$1 u@path dir_with_file.lst
\$2 (0|num_menu - file in dir)
CNTL: 
...
"
    ...
    _isn_od $num_menu && {
        _st_exit "in fs= file://${sh_file} , line=${LINENO}, EXEC: ${FNN} $* : NOT_NUMBER (\$2) : '$num_menu' : ${hint} : return 1"
        cd $PPWD
        return 1
    }
    ...
    _d2mm $ptr_path_1 result_ $num_menu >/dev/null
    ...
    for item in $(_f2e $result_); do
        # echo -e "${HLIGHT}--- _lcudr_tags2ue $item ---${NORMAL}" #start files
        for tag in $(_lcudr_tags2ue $item); do

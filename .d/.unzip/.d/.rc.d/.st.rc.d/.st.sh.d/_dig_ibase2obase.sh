...
_dig_ibase2obase() {
    ...
    local hint="digit \$1 in ibase \$2  to diggit on obase \$3"
    local item=
    ...
    if [[ "-h" == "$1" ]]; then
        echo -e "
MAIN: ${FNN} :: digit \$1 in ibase \$2  to diggit on obase \$3
TAGS: 
\$1 
[, \$2]
CNTL: 
...
"
    ...
    if _isn_od "$1"; then
        _st_exit "in fs= file://${sh_file} , line=${LINENO}, ${FNN}() : NOT_NUMBER : \$1 = '$1' : ${hint} : return 1"
        return 1
    fi

    if _isn_od "$2"; then
        _st_exit "in fs= file://${sh_file} , line=${LINENO}, ${FNN}() : NOT_NUMBER : \$2 = '$2' : ${hint} : return 1"
        return 1
    fi

    if _isn_od "$3"; then
        _st_exit "in fs= file://${sh_file} , line=${LINENO}, ${FNN}() : NOT_NUMBER : \$3 = '$3' : ${hint} : return 1"
        return 1
    fi

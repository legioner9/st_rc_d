...
RETU: 0 if any true search , 1 if all fail search
...
"
    ...
    while IFS=$'\n' read -r line; do
        ...
        for sub_str in ${gr_str_arr[@]}; do
            line=$(grep -E "$sub_str" <<<$line)
        done
        if [ -n "$line" ]; then
            echo $line
            flag_exist_str=0

...
_dr2Ae() {
    ...
    if [[ "-h" == "$1" ]]; then
        echo -e "
MAIN: ${FNN} :: recuce _d2e : like ls \$1 (-A, NOT uprev) (@|path) @ if path='' [, \$2 .ext]
TAGS:
\$1 
[, \$2]
CNTL: 
...
"
    ...
    if [[ -z "$1" ]]; then
        _st_exit "in _dr2Ae() : \$1 NOT_DEFINE : '$1', hint : '@ or path_to_dir' return 1"
        return 1
    fi
    ...
    if [[ "$1" == "@" ]]; then
        ...
        for item in $(ls -A); do
            if [ -z "$2" ]; then
                ...
                local _dr2Ae_ext=
                _dr2Ae_ext=$(_prs_f -e "$item")
                if { [ -d "$item" ] || [ -f "$item" ]; } && [ "${_dr2Ae_ext}" == "$2" ]; then
                    echo "$item"
                fi
                if [ -d "$item" ]; then
                    _dr2Ae "$item" "$2"
        ...
        for item in $(ls -A $1); do
            if [ -z "$2" ]; then
                ...
                local _dr2Ae_ext=
                _dr2Ae_ext=$(_prs_f -e "$item")
                if { [ -d "$1/$item" ] || [ -f "$1/$item" ]; } && [ "${_dr2Ae_ext}" == "$2" ]; then
                    echo "$1/$item"
                fi
                if [ -d "$1/$item" ]; then
                    _dr2Ae "$1/$item" "$2"

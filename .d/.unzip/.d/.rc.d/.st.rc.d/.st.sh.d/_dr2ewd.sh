...
_dr2ewd() {
    ...
    if [[ "-h" == "$1" ]]; then
        echo -e "
MAIN: ${FNN} :: _dfr2e - {echo \"file_path:depth_dir_contane_file\"} - \$1 dir (uprev) (@|path) @ if path='' [, \$2 .ext]
TAGS: 
\$1 
[, \$2]
...
"
    ...
    _dr2ewd_infn_1() {
        ...
        if [[ "$2" == "@" ]]; then
            ...
            ls $2 >/dev/null || {
                _st_exit "in fs= file:// , line=${LINENO}, ${FNN}() : : EXEC_FAIL : 'ls $2 >/dev/null' : ${hint} : return 1"
                return 1
            }
            for item in $(ls $2); do
                if [ -z "$3" ]; then
                    if { [ -f "$2/$item" ]; } && [ "${item:0:1}" != "_" ]; then
                        echo "$2/$item":$1
                    fi
                    if [ -d "$2/$item" ] && [ "${item:0:1}" != "_" ]; then
                        _dr2ewd_infn_1 $((arg1 + 1)) "$2/$item"
                    fi
                else
                    local _dr2e_ext=
                    _dr2e_ext=$(_prs_f -e "$item")
                    if { [ -f "$2/$item" ]; } && [ "${item:0:1}" != "_" ] && [ "${_dr2e_ext}" == "$3" ]; then
                        echo "$2/$item":$1
                    fi
                    if [ -d "$2/$item" ] && [ "${item:0:1}" != "_" ]; then
                        _dr2ewd_infn_1 $((arg1 + 1)) "$2/$item" "$3"
                    fi
                fi
            done
        fi

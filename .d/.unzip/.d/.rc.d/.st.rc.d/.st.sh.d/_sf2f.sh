...
[, \$2]
CNTL: 
    _e : body    : _edit ${HOME}/.d/.rc.d/.st.rc.d/.st.sh.d/_sf2f.sh
    _t : tst_dir : _edit ${HOME}/.d/.rc.d/.st.rc.d/.st.tst.d/_sf2f.tst.d
...
"
    ...
    if [[ "_e" == "$1" ]]; then
        _edit ${HOME}/.d/.rc.d/.st.rc.d/.st.sh.d/_sf2f.sh
        return 0
    fi

    if [[ "_t" == "$1" ]]; then
        _edit ${HOME}/.d/.rc.d/.st.rc.d/.st.tst.d/_sf2f.tst.d
        return 0
    fi
    ...
    if ! _abs_path "${PPWD}" init_file; then
        echo "in fs= file://${HOME}/.d/.rc.d/.st.rc.d/.st.sh.d/_sf2f .sh , line=${LINENO}, ${FNN}() : : EXEC_FAIL : '_abs_path ${PPWD} init_dir' : ${hint} : return 1" >&2
        return 1
    fi
    ...
    if ! [[ -f "$init_file" ]]; then
        echo "in fs= file://${HOME}/.d/.rc.d/.st.rc.d/.st.sh.d/_sf2f.sh , line=${LINENO}, ${FUNCNAME}() : NOT_FILE : 'file://$init_file' : ${hint} : return 1" >&2
        return 1
    fi

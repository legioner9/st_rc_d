...
_d4d() {
    ...
    if [[ "-h" == "$1" ]]; then
        echo -e "
MAIN: ${FNN} :: see .hie :: from insert dir \$1 for sdir: [{sub_dir_name} (\$2 pref_sdir) recurce recive dir \$3 replace sdir: [\$2{sub_dir_name}] ]
TAGS: 
\$1 inset dir
...
"
    ...
    local _recive_dir=
    _recive_dir=$(_prs_f -d ${recive_dir})/_$(_prs_f -ne ${recive_dir})
    echo -e "${GREEN}\$_recive_dir = '$_recive_dir'${NORMAL}" #print variable
    ...
    _d4d_insert() {
        ...
        for recive_sdir in $(_ddr2e "$recive_dir"); do
            recive_sdir_name=$(_prs_f -ne "${recive_sdir}")
            str_src_dst=${recive_sdir_name:1}
            ...
            eval "str_src_dst_arr=(\${str_src_dst//${prefix_sdir}/ })"
            src_name=${str_src_dst_arr[0]}
            dst_name=${str_src_dst_arr[1]}

            # echo -e "${GREEN}\$src_name = '$src_name'${NORMAL}"       #print variable
            # echo -e "${GREEN}\$dst_name = '$dst_name'${NORMAL}"       #print variable
            # echo -e "${GREEN}\$prefix_sdir = '$prefix_sdir'${NORMAL}" #print variable
            ...
            if [ "${prefix_sdir}" == "$2" ]; then
                ...
                for insert_sdir_name in $(_dd2e "$1"); do
                    insert_sdir=$insert_dir/${insert_sdir_name}
                    # echo -e "${GREEN}\$insert_sdir_name = '$insert_sdir_name'${NORMAL}" #print variable

                    #! имя из иттератора insert_sdir_name совпадает src_name полученного после парсинга str_src_dst
                    if [ "${insert_sdir_name}" == "${src_name}" ]; then
                        flag_not_change=1
                        ...
                        #! получаем путь sdir с корнем от recive_sdir и именем dst_name из парсинга str_src_dst
                        dst_sdir=$(_prs_f -d ${recive_sdir})/$dst_name

                        # echo -e "${GREEN}\$recive_sdir = '$recive_sdir'${NORMAL}" #print variable
                        #! делаем защищенную копию recive_sdir
                        ...
                        mkdir "${tmp_sdir}"
                        touch "${tmp_sdir}"/_0
                        #! создаем заново dst_sdir и копируем в нее содержание insert_sdir
                        rm -r ${dst_sdir}
                        mkdir ${dst_sdir}

                        cp -r ${insert_sdir}/. ${dst_sdir}

...
_lnv2d() {
    ...
    if [[ "-h" == "$1" ]]; then
        echo -e "
MAIN: ${FNN} :: insert list name value cntx from \$1 file.lst to dir (with prev) \$2
TAGS:
\$1 insert list name value cntx from \$1 file.lst
\$2 to dir \$2
CNTL: 
...
"
    ...
    hint="insert list name value cntx from \$1 file.lst to dir (with prev) \$2"
    ...
    for item in $(_f2e ${ptr_path_1}); do
        echo -e "${HLIGHT}--- _s2d {{$item}} \$$item ${ptr_path_2} ---${NORMAL}" #start files
        eval "_s2d {{$item}} \$$item ${ptr_path_2}"
    done
